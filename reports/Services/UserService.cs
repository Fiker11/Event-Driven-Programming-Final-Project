...
namespace Reports.Services{
    public class UserService : IUserService{
        ...
        public UserService(IConfiguration config){
            ...
            _userCollection = database.GetCollection<User>(config["MongoDbSettings:CollectionName"]);
        }
        ...
        public Task<List<User>> SearchUsers(string? name, string? email)
        {
            try{
                ...
                if (!string.IsNullOrEmpty(name)){
                    filter = Builders<User>.Filter.And(filter, Builders<User>.Filter.Regex(user => user.UserName, new MongoDB.Bson.BsonRegularExpression(name, "i")));
                }
                if (!string.IsNullOrEmpty(email)){
                    filter = Builders<User>.Filter.And(filter, Builders<User>.Filter.Regex(user => user.Email, new MongoDB.Bson.BsonRegularExpression(email, "i")));
                }
